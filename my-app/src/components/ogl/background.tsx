...
interface OrbProps {
  ...
  rotateOnHover?: boolean;
  forceHoverState?: boolean;
  primaryColor: [number, number, number]; // normalized RGB (0–1)
}
...
export default function Orb({
  ...
  useEffect(() => {
    ...
    // Derive 3 theme-based color variants
    const baseColor1 = primaryColor;
    const baseColor2 = primaryColor.map((c) => Math.min(c * 1.2, 1)) as [
      number,
      number,
      number,
    ];
    const baseColor3 = primaryColor.map((c) => c * 0.6) as [
      number,
      number,
      number,
    ];
  ...
  }, [hue, hoverIntensity, rotateOnHover, forceHoverState, primaryColor]);
